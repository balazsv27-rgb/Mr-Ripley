...
When applying the snapshot contract, use role-matched current-state authority.

### Primary current-state authority
1. `SYSTEM_TECHNICAL_HANDBOOK_v1.md` — core invariants, Layer-2 → Layer-3 interface rules, DecisionPacket anchor requirements
2. `SYSTEM_ARCHITECTURE_AND_BUILD_SEQUENCE_v1.md` — Layer-3 bootstrap rule, governed inputs, storage isolation, Event Risk non-direction rule
3. `README_v1.md` — system identity and high-level current-state properties

### Supporting current-state authority
4. `SYSTEM_LIMITATIONS_AND_APPROXIMATIONS_v1.md`
5. `DOCUMENTATION_VERIFICATION_MATRIX_v1.md` — classification status of snapshot-contract-related items
6. `SYSTEM_IMPLEMENTATION_RECORD_v1.md` — implementation-state / realized-build support within its declared role
7. `README_LAYER2.md` — collaborator guide and living build reference; may corroborate interface summaries within its declared role, but must not override Tier 1 contract authority

### CLAUDE.md
The project constitution defines the snapshot contract as a non-negotiable rule. Its rules govern all interpretation.
...
Source corroboration: `README_LAYER2.md` Section 6 (canonical collaborator/build-reference support; not an override of Tier 1 contract authority).
...
- [ ] `README_LAYER2.md` was used only within its declared collaborator/build-reference role and did not override Tier 1 contract authority
...
7. `README_LAYER2.md` was used only as role-scoped supporting contract corroboration within its declared collaborator/build-reference role, never as primary Tier 1 authority.
